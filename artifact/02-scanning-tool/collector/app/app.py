...
from config import QUERY_WORKERS, PARSER_WORKERS, SERVER_WORKERS, LOGS_FOLDER, QUERIES_PER_SECOND
...
def server_sshfp(dns_sshfpd_json):
    ...
    try:
        ...
        resp = dns.query.udp(req, where=dnssec_recursor_ip, timeout=10) # , timeout=5 ?
...
def init_domainfile():
    ...
    with open(os.environ['DOMAINFILE']) as csvfile:
        domainreader = csv.reader(csvfile, delimiter=',', quoting=csv.QUOTE_NONE)
