...
def sshfp_match_ratio():
	...
	cs=cm.Greys(np.arange(0.15, 0.9, (0.9-0.15)/float(n)))
	...
	values = [float(v) for v in SSHFP_MATCH_RATIO.values()]
	labels = [f"{int(round(k,2) * 100)}\% ({SSHFP_MATCH_RATIO[k]}, {round(SSHFP_MATCH_RATIO[k]/nsum*100,2)}\%)" for k in SSHFP_MATCH_RATIO]

	#pyplot.pie(values, labels=labels, autopct=None, colors=cs, labeldistance=1.7)
	pyplot.pie(values, colors=cs)
